"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PhotoController = void 0;
const PhotoService_1 = require("../services/PhotoService");
class PhotoController {
    constructor() {
        this.photoService = new PhotoService_1.PhotoService();
    }
    getPhotoDetails(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                console.log('getPhotoDetails');
                const { photoId } = req.params;
                const photoDetails = yield this.photoService.getPhotoDetails(photoId);
                res.json(photoDetails);
            }
            catch (error) {
                if (error instanceof Error) {
                    res.status(500).send(error.message);
                }
            }
        });
    }
    getPhotosWithFilters(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                console.log('getPhotosWithFilters');
                const filters = {
                    title: req.query.title,
                    albumTitle: req.query['album.title'],
                    userEmail: req.query['album.user.email']
                };
                console.log('filters', filters);
                console.log('req.query', req.query);
                const limit = !isNaN(parseInt(req.query.limit)) ? parseInt(req.query.limit) : 25;
                const offset = !isNaN(parseInt(req.query.offset)) ? parseInt(req.query.offset) : 0;
                const photos = yield this.photoService.getPhotosWithFilters(filters, limit, offset);
                res.json(photos);
            }
            catch (error) {
                if (error instanceof Error) {
                    res.status(500).send(error.message);
                } // testing
            }
        });
    }
}
exports.PhotoController = PhotoController;
